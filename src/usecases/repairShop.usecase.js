...
async function getById(id, repairShopId) {
  ...
  const repairShop = await RepairShop.findById(id).populate({
    path: "quotes",
...
async function updateStripeAccount(id, repairShopId) {
  ...
  if (!accountId) {
    throw createError(
      400,
      "Repair shop does not have an associated Stripe account ID."
    );
  }
  ...
  if (account.charges_enabled) {
    ...
    throw createError(
      400,
      "Stripe account activation is incomplete. Please complete the activation process in Stripe."
    );
  }
